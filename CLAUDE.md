...
对使用方的补丁依赖必须持续强提醒（README / CHANGELOG / migration / hotfix 四处均已写明）：
曾按 hotfix 方案手工加过 `httpclient5:5.2.3` / `httpcore5:5.2.4` / `httpcore5-h2:5.2.4` 的项目，
升级 beta3 后**必须删掉**这三个依赖。不是「留着也行」，理由有三：
1. `5.2.3` 落在 CVE-2026-64607 影响区间（`>=5.0-alpha1, <5.6.3`），留着等于钉死在有漏洞版本上
2. 直接声明优先级最高，将来使用方升级 Spring Boot 时会**静默顶掉** BOM 抬高的版本
3. 不删不会立刻报错，极易被遗忘，比显式报错更危险
...
- **中央仓库已发布的 15 个版本已全部补打 tag**（v1.0.0 ~ v1.1.0-beta3），见下「历史版本 tag」
- 历史遗留问题：旧提交信息里的 `1.1.0.9-GA` 四段式编号已废弃，从下个版本起统一切换到 SemVer，并确保 pom 版本与 tag 一致

## 历史版本 tag（已补齐，勿删勿移）

中央仓库共发布 15 个版本，均已打上带注释的 tag。**tag 不指向分支 tip，而指向真实构建点**——
多数老版本在发布之后分支上又落了提交，tip 与已发布 jar 并不一致（例：`1.0.9-GA` 的 tip
`39535f1` 与中央仓库 jar 有 2 处源码差异，真实发布点是 `9b777f1`）。

| tag | commit | tag | commit |
|-----|--------|-----|--------|
| v1.0.0 | 9de51e3 | v1.0.8 | 5a47171 |
| v1.0.1 | a5af016 | v1.0.9 | a8cb2bd |
| v1.0.2 | a6182b4 | v1.0.9-GA | 9b777f1 |
| v1.0.3 | 5a22eeb | v1.1.0 | 835e619 |
| v1.0.4 | 19cd8ab | v1.1.0-beta | 25261d3 |
| v1.0.5 | e3af5ad | v1.1.0-beta2 | 7e7ea07 |
| v1.0.6 | bd19961 | v1.1.0-beta3 | 0fbb1c5 |
| v1.0.7 | a674c00 | | |

未打 tag 的两个版本，**都不要补**：
- `1.0.9-SNAPSHOT`（`a83d764`）：快照版，从未发到中央仓库
- `1.1.1`（`e3b9f3b`/`1de06bd`/`1f5247c`/`70a4ba8`）：git 历史里存在过这个版本号，
  但**从未发布**。当时先跳到 1.1.1，后来退回 1.1.0-beta 走 beta 流程，这四个 commit 夹在
  beta/beta2/beta3 的血脉中间。1.1.1 留给下一个正式版用

### 定位历史发布点的方法（以后遇到同类问题照此办理）

光看 pom 版本无法定位发布点：pom-bump 之后的若干提交 pom 版本都一样。
正确做法是拿中央仓库的产物反查：

1. 下载该版本的 `-sources.jar`，与各候选 commit 的 `src/main/java` 逐一 `diff -r`
2. **源码一致只是必要条件，不是充分条件。** sources jar 不含文档和 pom，
   凡只改 README/pom 的提交都会「完全一致」——按「取最后一个匹配」会选到发布之后很久的提交
3. 再用 jar 内条目的构建时间定位：**取构建时间之前、且源码一致的最后一个 commit**。
   实测发布构建都发生在 pom-bump 提交前后几分钟内，分支 tip 往往在十几天后
4. zip 存的是本地时间、无时区，允许几分钟误差（1.0.3、1.0.6 的构建时间比提交早 1~4 分钟）
